$(document).ready(function () {
    const table = $("#assigned_table").DataTable({
...
//     const tableRows = document.querySelectorAll('#assigned_table tbody tr');
...
$("#columnVisibility").change(function () {
    ...
    if (Array.isArray(selectedColumns)) {
        // Show all columns
        $("#assigned_table th, #assigned_table td").show();

        // Hide selected columns
        selectedColumns.forEach(function (index) {
            $(
                `#assigned_table th:nth-child(${
                    parseInt(index) + 1
                }), #assigned_table td:nth-child(${parseInt(index) + 1})`
            ).hide();
        });
    } else {
