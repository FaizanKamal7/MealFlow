...
function assignBagCollections() {
    // Get all checkboxes
    const checkboxes = document.querySelectorAll('input.form-check-input[type="checkbox"]');
    // Initialize an array to store selected values
    const selectedEmptyBagCollectionIds = [];
    // Loop through checkboxes and add the selected values to the array
    checkboxes.forEach((checkbox) => {
        if (checkbox.checked) {
            selectedEmptyBagCollectionIds.push(checkbox.value);
        }
    });
    const driver_id = document.getElementById("driverSelect").value;

    var url = "/admin/deliveries/bag/collections/assign-bag-collection-to-driver/";
    ...
    $.ajax({
        ...
        data: { selected_empty_bag_collection_ids: selectedEmptyBagCollectionIds, driver_id: driver_id },
